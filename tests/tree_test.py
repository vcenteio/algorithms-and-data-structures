...
def test_node_instantiation_with_parent_and_children_with_subchildren(
    n3: TreeNode,
):
    parent = TreeNode("Living Organisms")
    genera = ["Canines", "Bovines", "Felines", "Birds"]
    ...
    expected_subchildren = {
        subchild for subgenus in subchildren for subchild in subgenus
    }
...
def test_node_pop_child_correct_value(n1: TreeNode):
    ...
    assert removed_child._parent is None
...
def test_node_siblings(n0: TreeNode, n1: TreeNode):
    ...
    i = 0
    for child in n1.children:
        assert child.siblings == tuple(
            (child for child in children if child != children[i])
        )
        i += 1
...
def test_node_neighbours(
    n0: TreeNode, n1: TreeNode, n2: TreeNode, n3: TreeNode
):
    ...
    for descendant in n3.descendants:
        assert (
            descendant not in n3.neighbours
            if descendant.level >= n3.level + 2
            else descendant in n3.neighbours
        )
...
def test_node_degree(n0: TreeNode, n1: TreeNode, n2: TreeNode, n3: TreeNode):
    ...
    for node in (n0, n1, n2, n3):
        for subnode in node.all_nodes:
            ...
            assert (
                subnode.degree == 0
                if subnode.is_leaf_node()
                else subnode.degree > 0
            )
...
def test_node_distance_from_descendant_correct_type(
    n0: TreeNode, n1: TreeNode, n2: TreeNode, n3: TreeNode
):
    with pytest.raises(ValueError):
    ...
    for child in n2.children:
        for subchild in child:
            assert (
                n2.get_distance_from_descendant(subchild)
                == 2
                == n2.get_distance_from_descendant(child) + 1
            )

    for node in n3.parent.descendants:
        assert (
            n3.parent.get_distance_from_descendant(node)
            == node.level - n3.parent.level
        )
...
def test_node_leaves(n0: TreeNode, n1: TreeNode, n2: TreeNode, n3: TreeNode):
    ...
    expected_n1_leaves = tuple(
        map(TreeNode, ["Canines", "Bovines", "Felines", "Birds"])
    )
...
def test_tree_set_root_correct_type(t2: Tree):
    root_parent = TreeNode("Animal")
    root = TreeNode("Mammal", root_parent)
    _tree = Tree(root)
    assert _tree.root == root
    assert _tree.root.parent == root.parent is None is not root_parent
...
def test_tree_lowest_common_ancestor_nodes_of_the_different_trees(
    t0: Tree, t1: Tree, t2: Tree, t3: Tree
):
    node1 = t3.find(TreeNode("Wolf"))
    ...
    assert t3.get_lowest_common_ancestor(node1, TreeNode("Dinosaur")) is None
